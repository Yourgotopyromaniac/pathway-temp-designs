...
- Icons: Lucide only, imported by canonical name (e.g. `CircleCheck`, not the deprecated alias `CheckCircle2`), because the mobile team uses `lucide-react-native` with the same names. `IconTooltips` (mounted in `AppShell`) shows the name on hover.
